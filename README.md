...
| `.claude/commands/agent-os/` | Agent OS v3 slash commands (`plan-product`, `discover-standards`, `index-standards`, `inject-standards`, `shape-spec`) for spec-driven development. Adapted from [buildermethods/agent-os](https://github.com/buildermethods/agent-os) (MIT, © Builder Methods). |
| `agent-os/` | Spec-driven workspace: `product/` (mission, roadmap, tech-stack), `standards/` + `index.yml` (project-specific tribal knowledge by area), and `specs/` (one folder per non-trivial change). See `agent-os/README.md`. |
| `NOTICE` | Attribution for the adapted Agent OS material. |
...
7. **Bootstrap the spec-driven workspace:** run `/plan-product` to fill in `agent-os/product/` (mission, roadmap, tech-stack). Once the codebase has real patterns, run `/discover-standards` per area to capture tribal knowledge into `agent-os/standards/`. See `agent-os/README.md` for the full workflow and how it relates to `.cursor/rules/` and `.cursor/skills/`.
